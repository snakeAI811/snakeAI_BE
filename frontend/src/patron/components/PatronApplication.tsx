...
function PatronApplication({ userRole }: PatronApplicationProps) {
  ...
  const handleApplyForPatron = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/v1/user/apply-patron', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.log('Patron application transaction:', result);
        setApplicationReason('');
        fetchApplicationStatus(); // Refresh application status
      } else {
        throw new Error('Failed to submit patron application');
      }
    } catch (error) {
  ...
  const handleApproveApplication = async (applicationId: string) => {
    setLoading(true);
    try {
      const response = await fetch('/api/v1/user/approve-patron', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          // TODO: Add authentication headers
      ...
      if (response.ok) {
        const result = await response.json();
        console.log('Patron approval transaction:', result);
        fetchPendingApplications(); // Refresh pending applications
      } else {
        throw new Error('Failed to approve patron application');
      }
    } catch (error) {
  ...
  const handleRejectApplication = async (applicationId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        const result = await response.json();
        console.log('Patron rejection transaction:', result);
        fetchPendingApplications(); // Refresh pending applications
      } else {
        throw new Error('Failed to reject patron application');
      }
    } catch (error) {
