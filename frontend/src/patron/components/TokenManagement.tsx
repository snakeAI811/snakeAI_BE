...
function TokenManagement({ userRole }: TokenManagementProps) {
  ...
  const handleLockTokens = async () => {
    ...
    try {
      const response = await fetch('/api/v1/user/lock-tokens', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({ 
          amount: Number(lockAmount) * 1000000000, // Convert to lamports
          role: userRole.role 
        })
      });
      ...
      if (response.ok) {
        ...
        console.log('Lock tokens transaction:', result);
        setLockAmount('');
  ...
  const handleUnlockTokens = async () => {
    ...
    try {
      const response = await fetch('/api/v1/user/unlock-tokens', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          // TODO: Add authentication headers
      ...
      if (response.ok) {
        const result = await response.json();
        console.log('Unlock tokens transaction:', result);
        fetchTokenInfo(); // Refresh token info
      } else {
  ...
  const handleClaimRewards = async () => {
    ...
    try {
      const response = await fetch('/api/v1/user/claim-yield', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          // TODO: Add authentication headers
      ...
      if (response.ok) {
        const result = await response.json();
        console.log('Claim yield transaction:', result);
        fetchTokenInfo(); // Refresh token info
      } else {
